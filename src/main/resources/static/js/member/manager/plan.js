...
$('#search-input').on('keyup', function (e) {
    ...
    if (e.key === "Enter") {
        ...
        if(search_option === "all" ){
            location.href = `/manager/plan`;

        }
        else if (search_word === "") {
            Swal.fire('검색어를 입력해주세요.', '', 'warning')
        } else {
            location.href = `/manager/plan?pageNo=1&search_word=${search_word}&search_option=${search_option}`;
        }
    }
})
...
function pageChange(element){ // 페이지 버튼 클릭할시
    ...
    if(search_word === "") { // 검색어 없을 경우 그냥 페이지만 변경한다.
        location.href = `/manager/plan?pageNo=${pageNo}`;
    }else if(search_word !== ""){ // 검색어가 있을 경우 페이지NO과 함꼐 검색어와 option도 계속 전달한다.
        location.href = `/manager/plan?pageNo=${pageNo}&search_word=${search_word}&search_option=${search_option}`;
